"use server"
import { db, users, student } from "@/db/schema";
...
async function sendVerificationEmail(email: string, token: string) {
  const verificationLink = `${process.env.NEXTURL}/verify-email?token=${token}`;
    ...
    html: `
      <div style="font-family: Arial, sans-serif; background-color: #f4f4f4; padding: 20px;">
        <div style="max-width: 500px; background-color: #ffffff; border-radius: 10px; padding: 20px; text-align: center; margin: auto; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);">
          <h2 style="color: #333;">Welcome to <span style="color: #007bff;">${process.env.APP_NAME}</span>!</h2>
